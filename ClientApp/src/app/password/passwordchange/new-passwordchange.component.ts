...
import {PasswordChangeService} from 'src/app/password/service/PasswordChange.service'
...
export class NewPasswordChangeComponent implements OnInit {
  ...
  constructor(private snackBar: MatSnackBar, private authService: AuthService,private PasswordChangeService: PasswordChangeService,private fb: FormBuilder, private router: Router,  private route: ActivatedRoute,private confirmService: ConfirmService) { 
    // this.files = [];
  }
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.PasswordChangeService.find(+id).subscribe(
        res => {
          console.log(res);
          if (res) {
  ...
  //  if (id) {
    ...
    console.log(this.PasswordUpdateForm.value)
      this.confirmService.confirm('Confirm Update message', 'Are You Sure Update  Item').subscribe(result => {
        if (result) {
          this.PasswordChangeService.updatePassword(this.PasswordUpdateForm.value).subscribe(response => {
