...
import { TrainingCrewService } from 'src/app/basic-setup/service/TrainingCrew.service';
@Component({
...
export class NewPasswordChangeComponent implements OnInit {
  ...
  constructor(private snackBar: MatSnackBar, private authService: AuthService,private TrainingCrewService: TrainingCrewService,private fb: FormBuilder, private router: Router,  private route: ActivatedRoute,private confirmService: ConfirmService) { 
    // this.files = [];
  }
  ...
  //  if (id) {
    ...
    console.log(this.PasswordUpdateForm.value)
      this.confirmService.confirm('Confirm Update message', 'Are You Sure Update  Item').subscribe(result => {
        if (result) {
          this.TrainingCrewService.updatePassword().subscribe(response => {
