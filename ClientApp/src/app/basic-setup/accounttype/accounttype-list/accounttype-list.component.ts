...
export class AccountTypeListComponent implements OnInit {
  ...
  displayedColumns: string[] = [ 'ser', 'accoutType', 'actions'];
  ...
  getAdminAuthorities() {
    this.isLoading = true;
    this.AccountTypeService.getAdminAuthorities(this.paging.pageIndex, this.paging.pageSize,this.searchText).subscribe(response => {
