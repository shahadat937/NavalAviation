...
@Component({
  selector: 'app-new-aircraftflying',
  templateUrl: './new-aircraftflying.component.html',
  styleUrls: ['./new-aircraftflying.component.sass']
})
export class NewAirCraftFlyingComponent implements OnInit {
  pageTitle: string;
  destination: string;
  btnText: string;
  ...
  displayedColumns: string[] = ['ser',  'airCraftName', 'date', 'crew','startupPlanned', 'startUp', 'duration', 'endurance', 'dup', 'actions'];
  paging = {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.pageTitle = 'Edit AC Flying Program';
      ...
      this.AirCraftFlyingService.find(+id).subscribe(
        res => {
          this.AirCraftFlyingForm.patchValue({
            ...
            opaOff: res.opaOff,
            pdf: res.pdf,
            startupPlanned:res.startupPlanned,
            landingTimePlanned:res.landingTimePlanned,
    ...
    if(this.role != this.userRole.SuperAdmin && this.role != this.userRole.FLGWG){
  ...
  calculateEndDate(status){
    var getDuration = this.AirCraftFlyingForm.get('duration').value;
    console.log('from planned');
    if(status==0){ // 0 for planned
      var startupPlanned = this.AirCraftFlyingForm.get('startupPlanned').value;
      this.calculateDuration(getDuration,startupPlanned,status);
    }else{ // 1 for actual
      var startUp = this.AirCraftFlyingForm.get('startUp').value;
      this.calculateDuration(getDuration,startUp,status);
    }

  }
  calculateDuration(durationTime,StartTime,timeStatus){
    // var getPlannedStart = this.AirCraftFlyingForm.get('startupPlanned').value;
    const [hours, minutes] = StartTime.split(':');
    var getTotalMins= (Number(hours*60)+Number(minutes));
    
    
    const [duhours, duminutes] = durationTime.split(':');
    var calculateDurationToMin = (Number(duhours*60)+Number(duminutes));
    var getTotalTime = Number(getTotalMins)+Number(calculateDurationToMin);
    console.log(getTotalTime);
    var ampmTime;
    var convertTime = String(getTotalTime/60);
    var convertMins= String(getTotalTime%60);
    const [conhours, conminutes] = convertTime.split('.');
    if(Number(conhours) > 24){
      ampmTime = Number(conhours)%24;
    }else{
      ampmTime =  Number(conhours);
    }

    if(ampmTime<10){
      ampmTime = '0'+ampmTime;
    }
    if(Number(convertMins)<10){
      convertMins = '0'+convertMins;
    }

    console.log(ampmTime);
    var plannedLandingTime = ampmTime+':'+convertMins;
    console.log(convertTime,convertMins)
    console.log(plannedLandingTime)

    // this.AirCraftFlyingForm.get('landingTimePlanned').setValue(plannedLandingTime);

    if(timeStatus==0){ // 0 for planned
      console.log('setting-',timeStatus);
      this.AirCraftFlyingForm.get('landingTimePlanned').setValue(plannedLandingTime);
    }else{ // 1 for actual
      console.log('setting-',timeStatus);
      this.AirCraftFlyingForm.get('endurance').setValue(plannedLandingTime);
    }


    // var cccc = 60*0.1;
    // console.log(hours, minutes);
    // console.log(conhours,conminutes);
    // console.log(getTotalMins);
    // console.log(conhours+':'+convertMins);
    
    // console.log('after duration');
    // console.log(duhours, duminutes);
    // console.log(calculateDurationToMin);
  ...
  onSubmit() {
    ...
    if (id) {
      this.confirmService.confirm('Confirm Update message', 'Are You Sure Update This  Item').subscribe(result => {
        ...
        if (result) {
          this.AirCraftFlyingService.update(+id, this.AirCraftFlyingForm.value).subscribe(response => {
            this.router.navigateByUrl('/admin/dashboard/add-aircraftflying');
