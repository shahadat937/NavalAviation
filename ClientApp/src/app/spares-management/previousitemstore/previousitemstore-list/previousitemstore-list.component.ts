...
export class PreviousItemStoreListComponent implements OnInit {
  ...
  displayedColumns: string[] = [ 'ser', 'departmentName', 'itemDetail', 'itemSerNo','itemCategory', 'deno', 'actions'];
  dataSource: MatTableDataSource<PreviousItemStore> = new MatTableDataSource();
  
  constructor(private snackBar: MatSnackBar, private PreviousItemStoreService: PreviousItemStoreService,private router: Router,private confirmService: ConfirmService) { }
  ...
  getPreviousItemStores() {
    this.isLoading = true;
    this.PreviousItemStoreService.getPreviousItemStores(this.paging.pageIndex, this.paging.pageSize,this.searchText).subscribe(response => {
