...
export class NewDemandComponent implements OnInit {
  ...
  getSelectedItemCategory() {
    this.DemandService.getSelectedItemCategory(this.masterData.sparescategory.spares).subscribe((res) => {
  ...
  getPartNoByDepartmentNameId(id: number) {
    this.itemDetailsService.getPartNoForSparesByDepartmentNameId(id,1).subscribe((res) => {
  ...
  onSubmit() {
    ...
    var DemandDate = this.DemandForm.get("demandDate").value;
    console.log(DemandDate);
    var isDemandDateNull = DemandDate;
    ...
    if(!isDemandDateNull){
      console.log("demand Date Null");
      this.DemandForm.get("demandDate").setValue(isDemandDateNull);
    }
    console.log(this.DemandForm.value);
    if (id) {
      this.confirmService
        ...
        .subscribe((result) => {
          if (result) {
            this.DemandService.update(+id, formData).subscribe(
              (response) => {
                this.router.navigateByUrl("/spares-management/add-demand");
                //this.router.navigateByUrl('/spares-management/add-demand');
