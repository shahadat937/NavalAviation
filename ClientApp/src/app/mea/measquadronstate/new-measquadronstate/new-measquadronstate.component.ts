...
export class NewMeaSquadronStateComponent implements OnInit {
      ...
      this.MeaSquadronStateService.find(+id).subscribe(
        res => {
          this.MeaSquadronStateForm.patchValue({          
            ...
            totalhouratDelivey:res.totalhouratDelivey,
            totalHouratOccation:res.totalHouratOccation,
            qty:res.qty,
            ataCode:res.ataCode,
            dateofInstall:res.dateofInstall,
            totalLandingCycles:res.totalLandingCycles,
            totalAcHour:res.totalAcHour,
            resonForRemoval:res.resonForRemoval,
            description:res.description,
            workOrderNo:res.workOrderNo,
            dateofSubmition:res.dateofSubmition,
            dateOfDiscrepancy:res.dateOfDiscrepancy,
            serNo:res.serNo,
  ...
  getselectedTrad(){
    this.MeaSquadronStateService.getselectedTrad().subscribe(res=>{
      this.selectedTrad=res
      console.log(this.selectedTrad);      
    });
  }
  getSelectedItemNameAndPattNo(){
    var departmentNameId =this.MeaSquadronStateForm.value['departmentNameId'];
    this.MeaSquadronStateService.getSelectedItemNameAndPattNo(departmentNameId).subscribe(res=>{
      this.selectedItemNameAndPattNo=res
      console.log(this.selectedItemNameAndPattNo);      
    });
  }
  getselectedConditionofItem(){
    this.MeaSquadronStateService.getselectedConditionofItem().subscribe(res=>{
      ...
      this.confirmService.confirm('Confirm Update message', 'Are You Sure Update This  Item?').subscribe(result => {
        ...
        if (result) {
          this.MeaSquadronStateService.update(+id,this.MeaSquadronStateForm.value).subscribe(response => {
            this.router.navigateByUrl('/mea/measquadronstate-list');
