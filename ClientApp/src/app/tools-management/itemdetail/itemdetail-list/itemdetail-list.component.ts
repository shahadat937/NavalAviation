...
export class ItemDetailListComponent implements OnInit {
  ...
  getItemDetailsForTools() {
    this.isLoading = true;
    this.ItemDetailService.getItemDetailsForTools(this.paging.pageIndex, this.paging.pageSize,this.searchText,this.masterData.sparescategory.tools).subscribe(response => {
      this.dataSource.data = response.items; 
      this.paging.length = response.totalItemsCount    
      this.isLoading = false;
    })
