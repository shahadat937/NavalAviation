...
@Component({
  selector: 'app-new-maintainencestate',
  templateUrl: './new-maintainencestate.component.html',
  styleUrls: ['./new-maintainencestate.component.sass']
})
export class NewMaintainenceStateComponent implements OnInit {
  pageTitle: string;
  destination:string;
  btnText:string;
      ...
      this.confirmService.confirm('Confirm Update message', 'Are You Sure Update This  Item?').subscribe(result => {
        ...
        if (result) {
          this.CallibrationStateService.update(+id,this.CallibrationStateForm.value).subscribe(response => {
            this.router.navigateByUrl('/maintenence-planning/callibrationstate-list');
