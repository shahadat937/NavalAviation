...
@Component({
  selector: "app-new-dailyairworthinessfrom",
  templateUrl: "./new-dailyairworthinessfrom.component.html",
  styleUrls: ["./new-dailyairworthinessfrom.component.sass"],
})
export class NewDailyAirworthinessFromComponent implements OnInit {
  pageTitle: string;
  destination: string;
  btnText: string;
  ...
  groupArrays: { fromCategory: string; datas: any }[];
  ...
  onDailyAirworthinessFromListByDepartmentNameSelectionChange() {
    ...
    this.DailyAirworthinessFromService.getDailyAirworthinessFromListByDepartmentName(
      ...
      const groups = this.dailyAirworthinessFromList.reduce((groups, datas) => {
        const fromCategory = datas.fromCategory;
        if (!groups[fromCategory]) {
          groups[fromCategory] = [];
        }
        groups[fromCategory].push(datas);
        return groups;
      }, {});

      // Edit: to add it in the array format instead
      this.groupArrays = Object.keys(groups).map((fromCategory) => {
        return {
          fromCategory,
          datas: groups[fromCategory],
        };
      });
            ...
            this.DailyAirworthinessFromService.update(+id, formData).subscribe(
              (response) => {
                this.router.navigateByUrl(
                  "/maintenence-planning/add-dailyairworthinessfrom"
                );
                // this.router.navigateByUrl('/maintenence-planning/add-dailyairworthinessfrom');
