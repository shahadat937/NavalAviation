...
{
    public class GetItemDetailListRequestHandler : IRequestHandler<GetItemDetailListRequest, PagedResult<ItemDetailDto>>
    {
        ...
        public GetItemDetailListRequestHandler(ISchoolManagementRepository<SchoolManagement.Domain.ItemDetail> ItemDetailRepository, IMapper mapper)
        {
            _ItemDetailRepository = ItemDetailRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<ItemDetailDto>> Handle(GetItemDetailListRequest request, CancellationToken cancellationToken)
        {
            ...
            UTOfficerCategories = UTOfficerCategories.OrderByDescending(x => x.ItemDetailId).Skip((request.QueryParams.PageNumber - 1) * request.QueryParams.PageSize).Take(request.QueryParams.PageSize);
